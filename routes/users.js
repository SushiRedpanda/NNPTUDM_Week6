...
let { postUserValidator, changePasswordValidator, validateResult } = require('../utils/validatorHandler')
let userController = require('../controllers/users')

let { checkLogin, checkRole } = require('../utils/authHandler.js')

// all user/role operations require authentication
...
router.post("/", checkRole('admin'), postUserValidator, validateResult,
  async function (req, res, next) {
    try {
      let newItem = await userController.CreateAnUser(
        req.body.username,
        req.body.password,
        req.body.email,          // was req.body.message before
        req.body.role
      )
      // populate cho đẹp
